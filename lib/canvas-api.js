...
exports.fetchCanvasStudents = () => {
  ...
  .then(res => res.body.map(student => {
    let modStudent =  {
      ...
      name: `${student.name.split(' ')[0].trim().toLowerCase()}-${student.name.split(' ')[1].trim()[0].toLowerCase()}`,
      canvasID: student.id,
    };
    return modStudent;
  }))
...
exports.canvasAPIPromiseChain = (labName) => {
  ...
  return exports.fetchCanvasStudents()
  .then(() => exports.fetchUngradedLabs())
  .then(() => exports.fetchSubmissionsOfEachLab())
  .then(() => console.log('do we have lab name?', labName));
};
