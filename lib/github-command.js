...
exports.cloneMainLabRepo = (url, makeStudentGitBranches) => {
  ...
  .catch(() => {
    ...
    .then(() => makeStudentGitBranches(url, exports.getPullNumbers));
  });
};
...
exports.makeStudentGitBranches = (url, getPullNumbers) => {
  ...
  .then(pullNumbers => {
    pullNumbers.forEach(pull => {
      debug(`success pulling down pull#${pull.number} from user ${pull.user}`);
      return childProcess.execAsync(`cd ${labName}; ${gitFetch}/${pull.number}/head:${pull.number}-${pull.user}`);
    })
  })
  .then(() => debug('All PRs pulled down.'))
  .catch(() => {
    if (!pullNumbers.length) debug('This repo has no PRs.');
    return;
  });
};
